        ...
        for i in range(len(self.__instrument_list)):
            ...
            if bars.__contains__(instrument):
                ...
                if self.__position is None:
                    if self.__end_date >= bar.getDateTime() >= self.__start_date:
                        self.__position = self.enterLong(instrument, 1000, True, True)
...
def run_strategy():
    ...
    for code in top_codes:
        if os.path.exists('data/2015-01-01-to-2015-12-31-' + code +
                '.csv'):
            feed.addBarsFromCSV(code, 'data/2015-01-01-to-2015-12-31-' + code +
                    '.csv')
            to_buy.append(code)
    print feed.getRegisteredInstruments()
    # Evaluate the strategy with the feed.
    myStrategy = MoneyFlowStrategy(feed, to_buy, start='2015-01-08', end='2015-12-31')
    myStrategy.run()
